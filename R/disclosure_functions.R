...
Stat_Round <- function(orig_data, var_choice, round_cond) {
  ...
  if (
    !is.character(var_choice) ||
      length(var_choice) == 0 ||
      !all(var_choice %in% colnames(orig_data))
  ) {
    stop(
  ...
  if (
    !is.numeric(round_cond) ||
      length(round_cond) != 1 ||
      round_cond <= 1 ||
      round_cond %% 1 != 0
  ) {
    stop(
      "Error: 'round_cond' must be a single positive whole integer greater than 1."
    )
  }
  ...
  r_data <- orig_data |>
    dplyr::mutate(dplyr::across(
      dplyr::all_of(num_var_choice),
      ~ tidyr::replace_na(., 999999999)
    ))
  ...
  r_data <- r_data |>
    dplyr::mutate(dplyr::across(
      dplyr::all_of(num_var_choice),
      ~ plyr::round_any(., round_cond, round)
    ))
...
#' @param orig_data A data.frame or tibble containing the input data to be swapped.
...
#' @return A data.frame or tibble with the swapping applied to the selected variables.  The original data is returned if no numeric variables are selected.
#' @export
#' @examples
#' # Example dataset
#' inp_data <- dummy_wide
...
Stat_Swap <- function(orig_data, var_choice, swap_cond) {
  ...
  swapped_data[, num_var_choice][
    swapped_data[, num_var_choice] == 999999999
  ] <- NA
...
Stat_Secondary_Supress <- function(
  ...
  if (
    !is.character(pri_var_choice) ||
      length(pri_var_choice) != 1 ||
      !pri_var_choice %in% colnames(orig_data)
  ) {
    stop(
  ...
  if (
    !is.character(sec_var_choice) ||
      length(sec_var_choice) < 2 ||
      !all(sec_var_choice %in% colnames(orig_data)) ||
      any(sec_var_choice %in% pri_var_choice)
  ) {
    stop(
  ...
  ## Primary Suppression performed first ----
  ...
  ps_data <- ps_data |>
    dplyr::mutate(dplyr::across(
      dplyr::all_of(ps_num_var_choice),
      ~ dplyr::coalesce(., 999999999)
    ))
  ...
  ss_data[, ss_num_var_choice] <- lapply(
    ss_data[, ss_num_var_choice],
    apply_suppression,
    sup_cond,
    char_supp,
    zero
  ...
  ss_data <- ss_data |>
    dplyr::mutate(
      SDC_count = rowSums(ss_data[, ss_num_var_choice] == char_supp)
    )
  ...
  ss_data <- ss_data |>
    dplyr::rowwise() |>
    dplyr::mutate(
      min_value_sec = dplyr::if_else(
        SDC_count == 1,
        min(
          as.numeric(dplyr::c_across(dplyr::all_of(ss_num_var_choice))),
          na.rm = TRUE
        ),
        NA_real_
      )
  ...
  ss_data <- ss_data |>
    dplyr::mutate(dplyr::across(
      dplyr::all_of(ss_num_var_choice),
      ~ dplyr::if_else(is.na(min_value_sec) | . != min_value_sec, ., char_supp)
    ))
