...
test_that("Valid inputs", {
  ...
  expect_identical(r_data$`20-24`, c(NA, 0, 24))
  expect_identical(r_data$`25-29`, c(9, NA, 27))
  expect_identical(r_data$`30-34`, c(15, 3, NA))
  expect_identical(r_data$`35-39`, c(12, 3, 21))
  expect_identical(r_data$`40-44`, c(21, 9, 36))
  expect_identical(r_data$`45-49`, c(3, 9, 39))
  expect_identical(r_data$`50-54`, c(15, 3, 27))
  expect_identical(r_data$`55-59`, c(9, 12, 27))
  expect_identical(r_data$Total, c(NA, 45, 222))
})

test_that("Invalid orig_data", {
  expect_error(
    Stat_Round(NULL, "20-24", 3),
    "Error: 'orig_data' must be provided, cannot be NULL, and must be a data.frame or tibble."
  )
  expect_error(
    Stat_Round(list(), "20-24", 3),
    "Error: 'orig_data' must be provided, cannot be NULL, and must be a data.frame or tibble."
  )
})
...
test_that("Invalid var_choice", {
  expect_error(
    Stat_Round(r_sample_data, NULL, 3),
    "Error: 'var_choice' must be a non-empty character vector and all columns must exist in 'orig_data'."
  )
  expect_error(
    Stat_Round(r_sample_data, "nonexistent", 3),
    "Error: 'var_choice' must be a non-empty character vector and all columns must exist in 'orig_data'."
  )
})
...
test_that("Invalid round_cond", {
  expect_error(
    Stat_Round(r_sample_data, "20-24", -10),
    "Error: 'round_cond' must be a single positive whole integer."
  )
  expect_error(
    Stat_Round(r_sample_data, "20-24", 0),
    "Error: 'round_cond' must be a single positive whole integer."
  )
  expect_error(
    Stat_Round(r_sample_data, "20-24", 10.5),
    "Error: 'round_cond' must be a single positive whole integer."
  )
})
...
test_that("No numeric variables selected", {
  r_vars <- "line"
  expect_warning(
    Stat_Round(r_sample_data, r_vars, 3),
    "No numeric variables have been selected. The original input data will be returned."
  )
})
...
test_that("Stat_Swap handles invalid var_choice", {
  expect_error(
    Stat_Swap(swap_sample_data, NULL, 2),
    "Error: 'var_choice' must be a non-empty character vector"
  )
  expect_error(
    Stat_Swap(swap_sample_data, "D", 2),
    "Error: 'var_choice' must be a non-empty character vector"
  )
})
...
test_that("Stat_Swap handles invalid swap_cond", {
  expect_error(
    Stat_Swap(swap_sample_data, "A", -1),
    "Error: 'swap_cond' must be a single positive whole integer"
  )
  expect_error(
    Stat_Swap(swap_sample_data, "A", 1.5),
    "Error: 'swap_cond' must be a single positive whole integer"
  )
})
...
test_that("Stat_Swap returns original data if no numeric variables are selected", {
  ...
  expect_identical(result, swap_sample_data)
})
...
test_that("Function handles non-data.frame input", {
  expect_error(
    Stat_Primary_Supress(NULL, "20-24", 3, TRUE),
    "Error: 'orig_data' must be provided, cannot be NULL, and must be a data.frame or tibble."
  )
  expect_error(
    Stat_Primary_Supress(list(), "20-24", 3, TRUE),
    "Error: 'orig_data' must be provided, cannot be NULL, and must be a data.frame or tibble."
  )
})
...
test_that("Function handles non-existent columns", {
  expect_error(
    Stat_Primary_Supress(ps_sample_data, "nonexistent", 3, TRUE),
    "Error: 'var_choice' must be a non-empty character vector and all columns must exist in 'orig_data'."
  )
})
...
test_that("Function handles invalid suppression character", {
  expect_error(
    Stat_Primary_Supress(ps_sample_data, "20-24", "x", 3, TRUE),
...
test_that("Function handles invalid suppression condition", {
  expect_error(
    Stat_Primary_Supress(ps_sample_data, "20-24", sup_cond = -1, zero = TRUE),
...
test_that("Function handles invalid zero suppression flag", {
  expect_error(
    Stat_Primary_Supress(ps_sample_data, "20-24", sup_cond = 3, zero = "yes"),
    "Error: 'zero' must be a single logical value."
  )
})

test_that("Function suppresses values correctly", {
  result <- Stat_Primary_Supress(
    ps_sample_data,
    c("20-24", "25-29"),
    3,
    "*",
...
test_that("Function preserves NA values", {
  result <- Stat_Primary_Supress(ps_sample_data, "35-39", 3, "*", TRUE)
  expect_true(is.na(result$`35-39`[1]))
})

test_that("Function returns original data if no numeric variables are selected", {
  result <- Stat_Primary_Supress(ps_sample_data, "Total", 3, "*", TRUE)
  expect_identical(result, ps_sample_data)
})
...
test_that("Stat_Secondary_Supress works correctly", {
  ...
  expect_identical(s_data$Total, c("*", "*", "*", 4, 5, NA))

  # Test secondary suppression
  expect_identical(s_data$`20-24`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`25-29`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`30-34`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`35-39`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`40-44`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`45-49`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`50-54`, c("*", "*", "*", 4, 5, NA))
  expect_identical(s_data$`55-59`, c("*", "*", "*", 4, 5, NA))

  # Test suppression character
  expect_identical(s_data$Total[1], s_char)

  # Test suppression condition
  expect_identical(s_data$Total[4], "4")

  # Test zero suppression
  expect_identical(s_data$Total[6], NA_character_)
})

test_that("Stat_Secondary_Supress handles invalid input correctly", {
  # Example dataset
  inp_data <- data.frame(
    Total = c(1, 2, 3, 4, 5, NA),
    `20-24` = c(1, 2, 3, 4, 5, NA),
